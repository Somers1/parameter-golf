...
class Hyperparameters:
    ...
    kv_latent = int(os.environ.get("KV_LATENT", 64))
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float,
                 qk_gain_init: float, q_latent: int, kv_latent: int):
        super().__init__()
        if dim % num_heads != 0:
    ...
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float,
        ...
        if has_attention:
            ...
            self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base,
                                            qk_gain_init, q_latent, kv_latent)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, model_dim: int, num_heads: int,
        ...
        # MTP heads (discarded after training)
        self.mtp_head_list = nn.ModuleList(
            [CastedLinear(model_dim, vocab_size, bias=False) for _ in range(mtp_heads)]
        ) if mtp_heads > 0 else nn.ModuleList()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if len(self.mtp_head_list) == 0:
            return main_loss

        # Multi-token prediction
        mtp_loss = torch.zeros((), device=x.device, dtype=torch.float32)
        for i, head in enumerate(self.mtp_head_list):
            offset = i + 2
            if offset >= seq_len:
                continue
...
def main() -> None:
    ...
    mtp_param_count = sum(p.numel() for p in mtp_params)
    n_params = all_params - mtp_param_count
    log0(f"model_params:{n_params} (mtp_heads:{mtp_param_count})")
