...
class Hyperparameters:
    ...
    ple_enabled = bool(int(os.environ.get("PLE_ENABLED", "0")))  # Mini Per-Layer Embeddings (Gemma 4 inspired)
    ple_dim = int(os.environ.get("PLE_DIM", 16))  # Per-layer embedding dimension (projected up to model_dim)
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,engram_gate,ple.scales",
    ).split(",")
    if pattern
)
...
class PerLayerEmbedding(nn.Module):
    """Mini Per-Layer Embeddings (inspired by Gemma 4 PLE).
    Each decoder layer gets a unique token embedding lookup that is projected
    up to model_dim and added as a residual.  All layers share one embedding
    table but each layer has its own learned scale, so the table learns
    layer-specialized token signals at minimal param cost.

    With vocab=1024, num_layers=11, ple_dim=16:
        embed table:  1024 * 11 * 16 = 180,224 params
        proj:         16 * 512       =   8,192 params  (shared across layers)
        scales:       11             =      11 params
        Total:        ~188K params   → ~141KB at int6
    """
    def __init__(self, vocab_size: int, num_layers: int, ple_dim: int, model_dim: int):
        super().__init__()
        self.num_layers = num_layers
        self.ple_dim = ple_dim
        self.embed = nn.Embedding(num_layers * vocab_size, ple_dim)
    ...
    def _vocab_size(self) -> int:
        return self.embed.num_embeddings // self.num_layers

    def forward(self, token_ids: Tensor, layer_idx: int) -> Tensor:
        """Return per-layer embedding residual for the given layer."""
        offset_ids = token_ids + layer_idx * self._vocab_size
        h = self.embed(offset_ids)
        h = self.proj(h)
        return h * self.scales[layer_idx].to(dtype=h.dtype)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Mini Per-Layer Embeddings (Gemma 4 inspired)
        self.ple = PerLayerEmbedding(vocab_size, num_layers, ple_dim, model_dim) if ple_enabled else None
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            if self.ple is not None:
                x = x + self.ple(input_ids, i)
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                v_embed=ve, v0=v0, ngram_emb=ngram_emb)
    ...
    def forward(self, input_ids, target_ids):
        ...
        for i in range(self.num_encoder_layers):
            if self.ple is not None:
                x = x + self.ple(input_ids, i)
            ve = self._get_ve(i, input_ids, ve_cache)
            x = self.blocks[i](x, x0, v_embed=ve, ngram_emb=ngram_emb)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            bi = self.num_encoder_layers + i
            if skips:
            ...
            if self.ple is not None:
                x = x + self.ple(input_ids, bi)
            ve = self._get_ve(bi, input_ids, ve_cache)
            x = self.blocks[bi](x, x0, v_embed=ve, ngram_emb=ngram_emb)
        x = self.final_norm(x)
        x_flat = x.reshape(-1, x.size(-1))
        targets = target_ids.reshape(-1)
...
def main() -> None:
    ...
    # Engram gates (inside blocks) are picked up by block_named_params above
    ...
    if base_model.ple is not None:
        tok_params.append({"params": [base_model.ple.embed.weight], "lr": token_lr, "base_lr": token_lr})
        scalar_params.append(base_model.ple.proj.weight)
        scalar_params.append(base_model.ple.scales)
    ...
    ple_params = sum(p.numel() for p in base_model.ple.parameters()) if base_model.ple is not None else 0
    log0(f"ple_enabled:{args.ple_enabled} ple_dim:{args.ple_dim} ple_params:{ple_params}")
